package com.xgx.syzj.datamodel;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.xgx.syzj.app.Api;
import com.xgx.syzj.base.BaseRequest;
import com.xgx.syzj.bean.Result;
import com.xgx.syzj.bean.StoreRecordBean;
import com.xgx.syzj.event.BaseDataEvent;
import com.xgx.syzj.event.EventCenter;
import com.xgx.syzj.utils.FastJsonUtil;
import com.xgx.syzj.widget.list.ListPageInfo;
import com.xgx.syzj.widget.list.PagedListDataModel;

import java.util.ArrayList;
import java.util.List;

/**
 * @author zajo
 * @created 2015年10月30日 11:01
 */
public class RecordsDataModel extends PagedListDataModel<StoreRecordBean> {

    public static final byte LIST_RECORD = 0x11;

    private static byte code;
    private int memberId;

    private BaseDataEvent<StoreRecordBean> data = new BaseDataEvent<>();

    public RecordsDataModel(int num, int memberId)
    {
        mListPageInfo = new ListPageInfo<>(num);
        this.memberId = memberId;
    }

    @Override
    protected void doQueryData()
    {
        code = LIST_RECORD;
        Api.getRecordList(memberId, new BaseRequest.OnRequestListener() {

            @Override
            public void onSuccess(Result result)
            {
                JSONObject object = JSON.parseObject(result.getResult());
                List<StoreRecordBean> list;
                if (result.getStatus() == 200) {
                    EventCenter.getInstance().post(result);
                    list = FastJsonUtil.json2List(object.getString("records"), StoreRecordBean.class);
                } else {
                    list = new ArrayList<>();
                }
                data.dataList = list;
                if (list != null && list.size() > 0) {
                    if (list.size() >= mListPageInfo.getNumPerPage()) {
                        data.hasMore = true;
                    } else {
                        data.hasMore = false;
                    }
                } else {
                    data.hasMore = false;
                    list = new ArrayList<>();
                }
                setRequestResult(data.dataList, data.hasMore);
                EventCenter.getInstance().post(list);
            }

            @Override
            public void onError(String errorCode, String message)
            {
                setRequestFail();
                EventCenter.getInstance().post(message);
            }
        });
    }


}
